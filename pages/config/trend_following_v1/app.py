...
from frontend.pages.config.trend_following_v1.user_inputs import user_inputs
...
initialize_st_page(title="TrendFollowing V1", icon="📊", initial_sidebar_state="expanded")
backend_api_client = get_backend_api_client()

get_default_config_loader("trend_following_v1")
# User inputs
inputs = user_inputs()
st.session_state["default_config"].update(inputs)

st.write("### Visualizing TrendFollowing Signals")
...
fig = make_subplots(rows=2, cols=1, shared_xaxes=True,
                    vertical_spacing=0.02, subplot_titles=('Candlestick', 'Volume', "MACD"),
                    row_heights=[0.8, 0.2])
